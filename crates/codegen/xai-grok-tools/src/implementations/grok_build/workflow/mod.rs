...
impl crate::types::tool_metadata::ToolMetadata for WorkflowTool {
    ...
    fn description_template(&self) -> &str {
        r##"Launch a workflow: a Rhai script that orchestrates subagents as one background run. Provide exactly one source: `name` (a registered workflow — built-in, or from the project `.grok/workflows/` or user `~/.grok/workflows/`), an inline `script`, or a `script_path`. Optionally pass `args` (bound to the script's `args`) and `agent_budget`, an absolute cap on cumulative child-agent calls: every agent(), pipeline() and parallel() item consumes one slot (schema retries do not); default 128. The host also caps live children per run (32 by default, host-configured) — larger parallel() panels are queued and still act as a barrier. The call returns immediately; progress appears in `/workflows`${%- if system_reminders_enabled %} and completion is reported automatically — do not poll or sleep-wait${%- endif %}.

        - Multiple independent branches -> parallel([() => ..., () => ...]). Pass thunks; never parallelize overlapping edits.
        - Multiple items must each pass through the same ordered stages -> pipeline(items, ...stages). Items may run concurrently; stages for one item run in order. Each stage receives (previous, item, index). For unlimited stage composition, pass a closure array: pipeline(items, [stage1, stage2, ...]).
        - A stage may synchronously compose a registered workflow -> workflow("name", args). The child runs inline in the same workflow run; its complete(value) becomes the stage result and shares the parent's budget, journal, and cancellation.
